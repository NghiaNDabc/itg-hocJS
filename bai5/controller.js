...
myApp.factory("DoanhSoFactory", function () {
  return {
    tinhTongDoanhSo: function (form) {
      let tong = 0;
      for (let i = 1; i <= 12; i++) {
        let key = "ds_thang_" + i;
        let value = parseFloat(form[key] || 0);
        tong += value;
      }
      return tong;
    },
...
myApp.controller("QlbhController", function ($scope, $filter) {
  $scope.list = [
    {
      id: 1,
      nam: 2021,
      loai_ke_hoach: "Kế hoạch ngày",
      kieu_thuc_hien: "DS nhân viên",
    ...
    },
    {
      id: 2,
      nam: 2021,
      loai_ke_hoach: "Kế hoạch tháng",
      kieu_thuc_hien: "DS khách hàng",
  ...
  $scope.filterLoaiKeHoach = "";
  $scope.filterKieuThucHien = "";
  $scope.filterNam = "";
  $scope.searchKeyword = "";
  $scope.search = function () {
    if ($scope.filterLoaiKeHoach != "") {
      $scope.filteredList = $scope.filteredList.filter(
        (i) => i.loai_ke_hoach == $scope.filterLoaiKeHoach
      );
    }
    if ($scope.filterKieuThucHien != "") {
      $scope.filteredList = $scope.filteredList.filter(
        (i) => i.kieu_thuc_hien == $scope.filterKieuThucHien
      );
    }
    if ($scope.filterNam != "") {
      $scope.filteredList = $scope.filteredList.filter(
        (i) => i.nam == $scope.filterNam
      );
    }
    ...
    $scope.filteredList = $scope.filteredList.filter(
      (i) =>
        i.nam == keyword ||
        i.kieu_thuc_hien.toLowerCase().includes(keyword) ||
  ...
  $scope.check = function (input) {
    $scope.list.forEach((element) => {
      if (
        ...
        element.kieu_thuc_hien == input.kieu_thuc_hien &&
